...
public class AppDialogPresenter extends BasePresenter<AppDialogView> {
    ...
    private int mId;
    private boolean mSkipViewLaunch;
    ...
    public void showDialog(CharSequence dialogTitle, Runnable onFinish) {
        ...
        // Some views (e.g. the Compose Settings tab) render dialogs in their own already-visible
        // Compose tree instead of a separate Activity - skip launching one in that case. Off by
        // default; only a caller that just set itself as the view via setView() would opt in.
        if (!mSkipViewLaunch) {
    ...
    /**
     * When true, showDialog() won't launch/foreground the registered AppDialogView Activity - the
     * caller is expected to have already called setView() with something that'll render the
     * categories itself (e.g. inline in an already-visible Compose screen). Remember to set this
     * back to false once done, otherwise every other dialog in the app (context menus, confirmations)
     * would silently stop appearing.
     */
    public void setSkipViewLaunch(boolean skip) {
        mSkipViewLaunch = skip;
    }
