...
Your account avatar sits in the top bar, so switching profiles is one click instead of a trip
through Settings.

---

## Toggling back to the old UI

The Compose UI is on by default — it's the whole point of this fork — but it's one toggle away
from SmartTube's original leanback interface: **Settings → AetherTube → New UI (beta)**.
